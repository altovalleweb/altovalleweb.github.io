class Map {
  ...
  initMap() {
    this.map = L.map("map").setView(
      [38.572122306730996, -0.0702209653455989],
      15
    );
  ...
  getCapas() {
    ...
    return capas.map((layer) => {
      return this.transformLayerToJson(layer);
    });
  }
  ...
  /**
   * @description carga un geojson como capa
   * @param {*} geoJson 
   */
  ...
  loadCapa(geoJson) {
    ...
    if (geoJson.properties.hasOwnProperty("radius")) {
      options = {
